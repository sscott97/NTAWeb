...
for (plate in plate_sheets) {
  ...
  # Build a named color vector matching FullLabel
  legend_labels <- setNames(unique(plate_data$FullLabel), unique(plate_data$Titration))

  plate_base <- ggplot(plate_data, aes(x = Dilution, y = Mean, color = FullLabel, group = Titration)) +
    geom_line() +
    geom_point() +
    geom_errorbar(aes(ymin = Min, ymax = Max), width = 0.2) +
    scale_x_discrete(labels = function(x) ifelse(x == "0", "NSC", x)) +
    scale_y_log10(breaks = scales::log_breaks(n = 6)) +
    scale_color_manual(values = colors_for_labels, name = "Sample - Pseudotype") +
