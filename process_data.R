...
process_plate <- function(sheet_name) {
  safe_read_cell <- function(range) {
    val <- tryCatch(
      suppressMessages(read_excel(excel_file, sheet = sheet_name, range = range, col_names = FALSE)[[1]]),
      error = function(e) NA
    )
  ...
  dilutions <- suppressMessages(
    read_excel(excel_file, sheet = sheet_name, range = "A5:A12", col_names = FALSE)[[1]]
  )

  cat("📊 Dilutions read from", sheet_name, ":", paste(dilutions, collapse = ", "), "\n")
  cat("   NA values?", any(is.na(dilutions)), "\n")

  data <- suppressMessages(
    read_excel(excel_file, sheet = sheet_name, range = "B5:M12", col_names = FALSE)
  )
  ...
  full_labels <- sapply(names(sample_ids), function(q) {
    ...
    if (is.na(sid) || sid == "" || is.na(psd) || psd == "") {
      ...
      return(paste(sid, psd, sep = " + ")) #####Seperate sample ID and pseudotype#####
    }
  })
...
summary_base <- ggplot(all_data, aes(x = Dilution, y = Mean, color = Titration, group = Titration)) +
  ...
  scale_y_log10(breaks = scales::log_breaks(n = 6), limits = c(10, NA)) +
...
for (plate in plate_sheets) {
  ...
  legend_labels <- sapply(unique(plate_data$Titration), function(q) {
    val <- unique(plate_data$FullLabel[plate_data$Titration == q])
    val <- val[!is.na(val) & val != ""]
    if (length(val) == 0) return(q)
    return(val[1])
  })
  names(legend_labels) <- unique(plate_data$Titration)
  ...
  plate_base <- ggplot(plate_data, aes(x = Dilution, y = Mean, color = Titration, group = Titration)) +
    ...
    scale_x_discrete(labels = function(x) ifelse(x == "0", "NSC", x)) +
    scale_y_log10(breaks = scales::log_breaks(n = 6), limits = c(10, NA)) +
    scale_color_manual(values = color_map, labels = legend_labels, name = "Pseudotype + Sample ID") +
